...
In brief, your implementation should generate an indexed mesh and render it using the WebGL function **`void gl.drawElements(mode, count, type, offset)`{: style="background-color: WhiteSmoke"}**. You should pay attention to the `type` parameter as the type **`gl.UNSIGNED_SHORT`{: style="background-color: WhiteSmoke"}** is the largest supported natively in WebGL 1.0. This will limit your mesh to having only 65536 vertices. If you want more, you will need to use the extension [`OES_element_index_uint`{: style="background-color: WhiteSmoke"}](https://developer.mozilla.org/en-US/docs/Web/API/OES_element_index_uint).
